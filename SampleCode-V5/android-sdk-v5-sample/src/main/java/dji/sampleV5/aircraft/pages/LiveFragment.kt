        ...
        if (!liveStreamVM.isStreaming()) {
            liveStreamVM.startStream(object : CommonCallbacks.CompletionCallback {
                override fun onSuccess() {
                    ...
                    ToastUtils.showShortToast("스트림 시작함!!!")
                }

                override fun onFailure(error: IDJIError) {
//                    ToastUtils.showLongToast(StringUtils.getResStr(R.string.msg_start_live_stream_failed, error.description()))
                    ToastUtils.showShortToast("스트림 실패!!!"+error.description().toString() )
                }
            });
        }
    }
